...
public class StaticPropertyGeneratorProcessor extends AbstractProcessor {
    ...
    private void generateProperties(TypeElement entity, StaticProperties config) {
        ...
        for (Element field : entity.getEnclosedElements()) {
            ...
            if (Arrays.stream(config.ignore()).noneMatch(ig -> ig.equalsIgnoreCase(originalName))) { properties.add(originalName); }
        }
        ...
        sb.append("    public static List<String> properties() { return Arrays.asList(")
